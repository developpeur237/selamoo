<?php
if (!function_exists('digi_get_content_widget_price')) :
    function digi_get_content_widget_price($args, $instance, $show = true) {
        $args = array(
        ...
        $instance = array('title' => isset($instance['title']) ? $instance['title'] : esc_html__('Price', 'digi-theme'));

        $classWrap = !$show ? ' hidden-tag' : '';

        if (!is_post_type_archive('product') && !is_tax(get_object_taxonomies('product'))) {
            return '<div id="' . $args['widget_id'] . '-ajax-wrap" class="nasa-filter-price-widget-wrap' . $classWrap . '" data-instance="' . esc_attr(json_encode($instance)) . '" data-args="' . esc_attr(json_encode($args)) . '"></div>';
        }
        ...
        $prices = digi_get_filtered_price();
        ...
        $res = '<div id="' . $args['widget_id'] . '-ajax-wrap" class="nasa-filter-price-widget-wrap' . $classWrap . '" data-instance="' . esc_attr(json_encode($instance)) . '" data-args="' . esc_attr(json_encode($args)) . '">';
        ...
        '<form method="get" action="' . esc_url($form_action) . '">' .
            '<div class="price_slider_wrapper">' .
                ...
                '<div class="price_slider_amount">' .
                    '<input type="text" id="min_price" name="min_price" value="' . esc_attr($min_price) . '" data-min="' . esc_attr($min) . '" placeholder="' . esc_attr__('Min price', 'digi-theme') . '" />' .
                    '<input type="text" id="max_price" name="max_price" value="' . esc_attr($max_price) . '" data-max="' . esc_attr($max) . '" placeholder="' . esc_attr__('Max price', 'digi-theme') . '" />' .
                    '<button type="submit" class="button">' . esc_html__('Filter', 'digi-theme') . '</button>' .
                    '<div class="price_label">' .
                        esc_html__('Price:', 'digi-theme') . ' <span class="from"></span> &mdash; <span class="to"></span>' .
                    '</div>' .
                    $fields .
                    '<div class="clear"></div>' .
                '</div>' .
                '<a href="javascript:void(0);" class="reset_price' . $class_reset . '">' . esc_html__('Reset', 'digi-theme') . '</a>' .
            '</div>' .
            '<input type="hidden" class="nasa_hasPrice" name="nasa_hasPrice" value="' . $hasPrice . '" />' .
        '</form></div>';
...
if (!function_exists('digi_get_filtered_price')) :
    function digi_get_filtered_price() {
...
if (!function_exists('digi_get_content_widget_variation')) :
    function digi_get_content_widget_variation($args, $instance, $agrs_ajax = array()) {
        ...
        if (!taxonomy_exists($taxonomy)) {
            return '<div id="' . $args['widget_id'] . '-ajax-wrap" class="nasa-filter-variations-widget-wrap" data-instance="' . esc_attr(json_encode($instance)) . '" data-args="' . esc_attr(json_encode($args)) . '"></div>';
        }

        $content = '<div id="' . $args['widget_id'] . '-ajax-wrap" class="nasa-filter-variations-widget-wrap" data-instance="' . esc_attr(json_encode($instance)) . '" data-args="' . esc_attr(json_encode($args)) . '">';
        ...
        $term_counts = digi_get_filtered_term_product_counts(wp_list_pluck($terms, 'term_id'), $taxonomy, $query_type, $agrs_ajax, $ajax);
        ...
        if (0 < $count_terms) {
            ...
            $content .= '<ul class="nasa-variations-' . $vari_type . '">';
            ...
            $_chosen_attributes = $disable_ajax_product ? WC_Query::get_layered_nav_chosen_attributes() : false;
            foreach ($terms as $k => $term) {
                ...
                if($disable_ajax_product) {
                    ...
                    $link = digi_get_page_base_url($taxonomy);
                ...
                $content .= '<li class="' . $liClass . $class . ' nasa-attr-' . $attr . ' nasa_' . $attr . '_' . esc_attr($term->term_id) . '">';
                ...
                $content .= $vari_type == 'color' ? '<span class="nasa-filter-color-border"><span class="nasa-filter-color-span" ' . $style . '></span></span>' : '';
                
                if($vari_type == 'image' && $nasa_attr_ux_exist) {
                    $content .= '<span class="nasa-filter-image-border margin-right-10"><span class="nasa-filter-image-span">' . Nasa_Abstract_WC_Attr_UX::get_image_preview($term_meta, false, 28, 28) . '</span></span>';
                }
                
                $content .= $content_text;
                $content .= isset($instance['count']) && $instance['count'] ? ' <span class="' . $countClss . '">(' . $count . ')</span>' : '';
                $content .= '</a></li>';
            }

            $fadeIn = (isset($instance['effect']) && $instance['effect'] == 'fade') ? '1' : '0';
            $content .= ($show_items && ($count_terms > $show_items)) ?
                '<li class="nasa_show_manual" data-fadein="' . $fadeIn . '">' .
                    '<a data-show="1" class="nasa-show" href="javascript:void(0);">' .
                        esc_html__('+ Show more', 'digi-theme') .
                    '</a>' .
                    '<a data-show="0" class="nasa-hidden" href="javascript:void(0);">' .
                        esc_html__('- Show less', 'digi-theme') .
                    '</a>' .
                '</li>' : '';
 ...
 */
if (!function_exists('digi_get_filtered_term_product_counts')) :
    function digi_get_filtered_term_product_counts($term_ids, $taxonomy, $query_type, $args = array(), $ajax = false) {
        global $wpdb;
 ...
 * @return int
 */
if (!function_exists('digi_get_current_term_slug')) :
    function digi_get_current_term_slug() {
 ...
 * @return string
 */
if (!function_exists('digi_get_page_base_url')) :
    function digi_get_page_base_url($taxonomy) {
 ...
 */
if (!function_exists('digi_remove_default_wg_woo')) :
    function digi_remove_default_wg_woo() {
        global $nasa_opt;
