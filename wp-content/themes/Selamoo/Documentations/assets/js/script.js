...
jQuery(document).ready(function($) {
	...
	function activateNav(pos){
		var offset = 100,
		current, next, _parent, isSub, hasSub;
		win.unbind('hashchange', hashchange);
		for(var i=sectionscount;i>0;i--){
			if(sections[i-1].pos <= pos+offset){
				...
				_parent = current.parent().parent();
				next = current.next();
				
				hasSub = next.is('ul');
				isSub = !_parent.is('#documenter_nav');
				
				nav.find('ol:visible').not(_parent).slideUp('fast');
				if(isSub){
					_parent.prev().addClass('current');
					_parent.stop().slideDown('fast');
				}else if(hasSub){
