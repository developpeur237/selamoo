 ...
 *
 */
add_action('init', 'nasa_portfolio_init', 1);
...
function nasa_get_recent_portfolio($limit, $title = 'Recent Works', $not_in = 0) {
    global $nasa_opt;
    if(!isset($nasa_opt['enable_portfolio']) || !$nasa_opt['enable_portfolio']) {
        return '';
    }
...
function nasa_get_image($attachment_id = 0, $width = null, $height = null, $crop = true, $post_id = null, $get_sizes = false) {
    ...
    return !$attachment_id ? null : nasa_get_resized_url($attachment_id, $width, $height, $crop, $get_sizes);
}
...
function nasa_portfolio_scripts() {
     ...
     * Masonry
     */
    wp_enqueue_script('jquery-masonry-isotope', NASA_CORE_PLUGIN_URL . 'assets/js/min/jquery.masonry-isotope.min.js', array('jquery'), false, true);
}