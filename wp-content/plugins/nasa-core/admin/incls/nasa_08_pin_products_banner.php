<?php
add_action('manage_blocks_posts_custom_column', 'nasa_manage_pin_products_banner_columns', 10, 2);
function nasa_manage_pin_products_banner_columns($column, $post_id) {
    switch ($column) {
        case 'shortcode' :
        default :
            echo (int) $post_id ? '<span style="background:#eee;font-weight:bold;"> [nasa_pin_products_banner pin_id="' . $post_id . '" marker_style="price|plus"] </span>' : '';
            break;
    }
}
        ...
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                <?php if (isset($_GET["post"]) && $_GET["post"]): ?>
                    var item_id = $('input#post_ID').val();
                    if ($('#original_post_status').val() === 'publish') {
                        $('#submitpost #minor-publishing').append('<div class="misc-pub-section shortcode-info"><span><i class="fa fa-code" style="font-size: 1.2em; margin-right: 5px;"></i> Shortcode: <b>[nasa_pin_products_banner pin_id="' + item_id + '" marker_style="price|plus"]</b></span></div>');
                    }
                <?php endif; ?>
                    ...
                    if ($('input[name="post_status"]').val() !== 'trash') {
                        ...
                        $('#posts-filter table.wp-list-table tbody tr').each(function () {
                            if ($(this).hasClass('status-publish')) {
                                ...
                                $(this).append('<td data-colname="Shortcode"><b>[nasa_pin_products_banner pin_id="' + _post_id + '"]</b></td>');
                            } else {
                                $(this).append('<td></td>');
                            }
                        });
    ...
    if ('nasa_pin_pb' == $post_type) {
        if (in_array($pagenow, array('post.php', 'post-new.php'))) {
            if (!isset($_REQUEST['action']) || 'trash' != $_REQUEST['action']) {
                wp_enqueue_script('nasa_pin_pb_easing', NASA_CORE_PLUGIN_URL . 'assets/js/min/jquery.easing.min.js');
                wp_enqueue_script('nasa_pin_pb_easypin', NASA_CORE_PLUGIN_URL . 'assets/js/min/jquery.easypin.min.js');
