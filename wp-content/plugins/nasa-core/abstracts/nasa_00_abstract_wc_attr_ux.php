...
abstract class Nasa_Abstract_WC_Attr_UX {
    ...
    const _NASA_COLOR = 'color';
    const _NASA_LABEL = 'label';
    const _NASA_IMAGE = 'image';
     ...
     * @return string imgae
     */
    public static function get_image_preview($value = false, $id = false, $width = 60, $height= 60) {
        $image_src = $value ? wp_get_attachment_thumb_url($value) : false;
        $image_src = $image_src ? $image_src : self::$no_image;
        
        return '<img' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="attr-image-preview" src="' . esc_url($image_src) . '" width="' . (int) $width . '" height="' . (int) $height . '" />';
    }
